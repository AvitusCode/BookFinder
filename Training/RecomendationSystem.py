...
class ContentBasedRecommender(object):
    ...
    def recommend_on_movie(self, movie_title, n_recommends=5):
        self.is_hist = True
        movie_id = int(self.movies[self.movies['title'] == movie_title]['movieId'])
...
def get_recommendation_system():
    # TODO: read file with film collection
    movies = pd.read_csv('movies.csv', sep=';', encoding='utf8').drop('Unnamed: 3', axis=1)
    # TODO: read file with user rating collection
    ratings = pd.read_csv('ratings.csv', sep=';')
    # TODO: here we need to get ration pivot
